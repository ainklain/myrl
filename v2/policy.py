...
def main():
    ...
    model_name = 'meta_ppo_shared_invest_n_newenv2'
    ...
    for t_step, t in enumerate(range(t_start, t_start + 1)):
        ...
        for ep in range(EP_MAX + 1):
            ...
            model.metatrain(env_samples,  n_fast_train=5, n_tr_samples=5)
            e_train = time.time()
            ...
            if ep % 20 == 0:
                for j in range(len(env_samples)):
                    model.fast_train(env_samples[j], n_train=5, n_tr_samples=5)
                    ...
                    env.render(statistics=True, save_filename=img_path + "ep_{}_envt_{}.png".format(1860 + ep, env_samples[j]))
